...
class OrveaCrmLead(models.Model):
    ...
    state_transaction = fields.Selection([
    ...
    client_account = fields.Many2one('res.partner', string='Client Account')
    ...
    flag_details = fields.Boolean(default=False)
    lead_name = fields.Char(compute="_get_name_lead", store=True)
    mo_identification = fields.Many2one('res.partner',string="Identification MO", default=lambda self: self.env.user.partner_id.id)
...
#TODO: VERIFY ID UNIQUE FOR CONTACT&COMPANY
    ...
    def get_data_info(self):
        ...
        sale_order_form_view = self.env['ir.ui.view'].sudo().search([('model', '=', 'sale.order'),('name', '=', 'sale.order.form')])
...
#TODO: Create the Quotation View
    ...
    def action_create_quotation(self):
    	...
    	#form_view_id = self.env.ref('orvea_lead.similar_detail_tree_view').id       
        return {
			'name': "Nouveau Devis",
			'view_type': 'form',
